...
export class TaskModel {
  ...
  static findByOwnerProjects(ownerId: string): Task[] {
    return query(
      'SELECT t.* FROM tasks t INNER JOIN projects p ON t.project_id = p.id WHERE p.owner_id = ? ORDER BY t.created_at DESC',
      [ownerId]
    ) as Task[];
  }
