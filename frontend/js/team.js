...
(() => {
  ...
  var SESSION_EXPIRED_MESSAGE = "Your session has expired. Please log in again.";
  ...
  function renderMembers(members) {
    ...
    list.innerHTML = members.map((m) => {
      ...
      const roleBadge = m.role === "manager" ? `<span class="project-status" style="color:var(--accent)">Manager</span>` : `<span class="project-status">Member</span>`;
      const removeBtn = isManager && !isMe ? `<button type="button" class="secondary-button remove-member-btn" data-member-id="${escapeHtml(m.id)}" data-member-name="${escapeHtml(m.name)}" style="margin-top:12px;font-size:13px;">Remove</button>` : "";
  ...
  async function handleInviteSubmit(event) {
    ...
    let valid = true;
    if (!name) {
      setFieldError("invite-name", "Name is required.");
      valid = false;
    }
    if (!email) {
      setFieldError("invite-email", "Email is required.");
      valid = false;
    }
    if (!password || password.length < 6) {
      setFieldError("invite-password", "Password must be at least 6 characters.");
      valid = false;
    }
    if (!valid) return;
  ...
  function setupEventListeners() {
    ...
    document.getElementById("sidebar-backdrop")?.addEventListener("click", (e) => {
      if (e.target.dataset.closeSidebar === "true") closeSidebar();
    });
    ...
    document.getElementById("invite-modal")?.addEventListener("click", (e) => {
      if (e.target.dataset.closeModal === "true") closeInviteModal();
    });
